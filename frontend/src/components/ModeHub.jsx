...
import { ArrowLeft, CheckCircle2, CircleDashed, CircleOff } from "lucide-react";
...
export function ModeHub({ dayKey, i18n, onBackHome, onSelectMode }) {
  const { t } = i18n;
  ...
  const { statusByMode, recommendedModeId, handleSelectMode } = useHomeDashboard({
    dayKey,
    modeCatalog: modes,
    t,
  });
    ...
    <div className="min-h-screen bg-app text-app">
      <div className="mx-auto w-full max-w-[1900px] px-4 md:px-10 2xl:px-16 py-10 md:py-14">
        <button
          type="button"
          onClick={onBackHome}
          className="mb-6 inline-flex items-center gap-2 rounded-full border border-app bg-surface px-4 py-2 text-sm font-semibold text-strong hover:bg-surface-soft"
        >
          <ArrowLeft className="h-4 w-4" />
          {t("home.mode_hub.back_home")}
        </button>

        <div className="mb-6">
          <h2 className="text-2xl md:text-3xl font-black text-strong">{t("home.mode_hub.title")}</h2>
          <p className="text-sm text-muted mt-1">{t("home.mode_hub.subtitle")}</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6">
          {modes.map((m) => {
            const st = statusByMode[m.id] || { attempts: 0, won: false, played: false };

            return (
              <button
                key={m.id}
                onClick={() => handleSelectMode(m.id, onSelectMode)}
                className={[
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3 md:gap-5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="text-xl font-extrabold leading-tight break-words">{m.title}</div>
                ...
                <div className="mt-6 flex items-center justify-between text-sm">
                  <div className="text-muted">
                    {t("home.attempts")} <span className="font-semibold text-strong">{st.attempts}</span>
                  </div>

                  <div className="inline-flex items-center gap-2 text-muted transition group-hover:text-strong">
                    {t("home.play")} <span className="translate-x-0 group-hover:translate-x-0.5 transition-transform">→</span>
                  </div>
                </div>
              </button>
            );
